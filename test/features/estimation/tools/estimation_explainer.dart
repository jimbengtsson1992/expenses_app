...
class EstimationExplainer {
  ...
  String explain(
    ...
    buffer.writeln('\n========================================');
    buffer.writeln(
      'ESTIMATE DEBUG REPORT: ${category.name}/${subcategory.name}',
    );
    buffer.writeln(
    ...
    final matchingRecurring = recurring
        .where((r) => r.category == category && (r.subcategory == subcategory))
        .toList();
    ...
    buffer.writeln(
      'Total Recurring Estimate: ${recurringTotal.toStringAsFixed(2)}\n',
    );
    ...
    if (monthlyTotals.isEmpty) {
      ...
      if (showDetails && subHistory.isEmpty) {
        buffer.writeln(
          '  (No transactions found for this subcategory in history)',
        );
      } else if (showDetails) {
        buffer.writeln(
          '  (Transactions exist but were all filtered out by excludedFromEstimates or excludeFromOverview)',
        );
      }
    } else {
      double totalSum = 0;
      ...
      if (!anyRecurringForSub) {
        ...
        final currentDay =
            (targetYear == _now.year && targetMonth == _now.month)
            ? _now.day
            : daysInMonth;
        ...
        */
        ...
        if (remainingRatio == 0 &&
            (targetYear > _now.year ||
                (targetYear == _now.year && targetMonth > _now.month))) {
          buffer.writeln(
            '  WARNING: Future month variable estimate is 0 due to service logic (currentDay=daysInMonth).',
          );
        }
        ...
        buffer.writeln(
          '  Remaining Ratio: ${remainingRatio.toStringAsFixed(4)}',
        );
        buffer.writeln(
      ...
      if (showDetails) {
        ...
        for (final key in sortedMonthKeys) {
          buffer.writeln(
            'Month $key: Total ${monthlyTotals[key]!.toStringAsFixed(2)}',
          );
